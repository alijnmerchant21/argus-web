...
export const Route = createFileRoute("/dashboard")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Define and manage Argus rules." },
    ],
  }),
...
const ACTION_CONFIG: Record<
  RuleAction,
  { label: string; desc: string; Icon: React.ElementType; pastel: string; active: string }
> = {
  block: {
    label: "Block",
    desc: "Prevent the message from being sent",
    Icon: ShieldBan,
    pastel: "border-rose-200/80  bg-rose-50   text-rose-700",
    active: "border-rose-400  bg-rose-100  text-rose-800  ring-2 ring-rose-300",
  },
  warn: {
    label: "Warn",
    desc: "Show a warning — user can still proceed",
    Icon: AlertTriangle,
    pastel: "border-amber-200/80 bg-amber-50  text-amber-700",
    active: "border-amber-400 bg-amber-100 text-amber-800 ring-2 ring-amber-300",
  },
  flag: {
    label: "Flag",
    desc: "Log the event and show a lightweight flag indicator",
    Icon: Flag,
    pastel: "border-sky-200/80   bg-sky-50    text-sky-700",
    active: "border-sky-400   bg-sky-100   text-sky-800   ring-2 ring-sky-300",
  },
};
...
const TEMPLATE_GROUPS: {
...
}[] = [
  {
    label: "Doctors",
    icon: Stethoscope,
    pastel: "bg-sky-50 border-sky-200",
    templates: [
      {
        title: "No Patient PII",
        domain: "Medical",
        action: "block",
        severity: "high",
        keywords: ["patient name", "DOB", "medical record", "SSN"],
        body: "Sharing patient-identifying information with AI tools is not permitted.",
        notes: "HIPAA compliance",
        scope: "input",
        match_logic: "any",
      },
      {
        title: "Flag Diagnosis Requests",
        domain: "Medical",
        action: "flag",
        severity: "medium",
        keywords: ["diagnose", "diagnosis", "do I have", "symptoms of"],
        body: "",
        notes: "Flag AI-generated diagnoses for review",
        scope: "output",
        match_logic: "any",
      },
      {
        title: "Medication Dosage Warning",
        domain: "Medical",
        action: "warn",
        severity: "high",
        keywords: ["dosage", "mg", "milligram", "prescribe", "prescription"],
        body: "Verify all dosage information with a licensed pharmacist or physician.",
        notes: "",
        scope: "output",
        match_logic: "any",
      },
  ...
  },
  {
    ...
    templates: [
      {
        title: "Flag Client Info Sharing",
        domain: "Legal",
        action: "flag",
        severity: "high",
        keywords: ["client name", "case number", "settlement", "confidential"],
        body: "",
        notes: "Privilege protection",
        scope: "input",
        match_logic: "any",
      },
      {
        title: "Jurisdiction Advice Warning",
        domain: "Legal",
        action: "warn",
        severity: "medium",
        keywords: ["in your state", "legally you can", "you are entitled"],
        body: "AI legal advice is not jurisdiction-verified. Always consult a qualified attorney.",
        notes: "",
        scope: "output",
        match_logic: "any",
      },
  ...
  },
  {
    ...
    templates: [
      {
        title: "Flag Fabricated Citations",
        domain: "Research",
        action: "flag",
        severity: "high",
        keywords: ["according to", "study shows", "published in", "researchers found"],
        body: "",
        notes: "Hallucinated citations are a known risk",
        scope: "output",
        match_logic: "any",
      },
      {
        title: "Warn Dataset Hallucination",
        domain: "Research",
        action: "warn",
        severity: "medium",
        keywords: ["dataset", "sample size", "p-value", "statistically"],
        body: "Verify all statistical claims against primary sources.",
        notes: "",
        scope: "output",
        match_logic: "any",
      },
  ...
  },
  {
    ...
    templates: [
      {
        title: "Sensitive Policy Leak Warning",
        domain: "Policy",
        action: "warn",
        severity: "high",
        keywords: ["draft legislation", "internal memo", "embargoed", "off the record"],
        body: "This content may be sensitive. Confirm it is approved for AI processing.",
        notes: "",
        scope: "input",
        match_logic: "any",
      },
  ...
  },
  {
    label: "Academists",
    icon: GraduationCap,
    pastel: "bg-rose-50 border-rose-200",
    templates: [
      {
        title: "Flag Ghostwriting",
        domain: "Academia",
        action: "flag",
        severity: "high",
        keywords: ["write my essay", "write my paper", "write this for me", "do my homework"],
        body: "",
      ...
      },
      {
        ...
        action: "warn",
        severity: "medium",
        keywords: ["copy this", "rewrite verbatim", "exact words"],
        body: "Submitting AI-generated content as original work may violate academic integrity policies.",
        notes: "",
        scope: "input",
        match_logic: "any",
      },
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Define everything yourself from scratch.
              </p>
            </div>
          </button>
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 text-xs text-muted-foreground line-clamp-1">
                        {t.action === "block" ? "Blocks" : t.action === "warn" ? "Warns" : "Flags"}{" "}
                        · {t.severity} severity
                        {t.keywords.length > 0 &&
                          ` · "${t.keywords[0]}"${t.keywords.length > 1 ? ` +${t.keywords.length - 1}` : ""}`}
                      </p>
                    </div>
                    <span
                      className={cn(
...
function KeywordInput({
  ...
  keywords: string[];
  keywordActions: Record<string, RuleAction>;
  defaultAction: RuleAction;
  onChange: (k: string[]) => void;
  onActionChange: (k: Record<string, RuleAction>) => void;
  readOnly: boolean;
}) {
  const [draft, setDraft] = useState("");
  const [targetAction, setTargetAction] = useState<RuleAction>(defaultAction);
  ...
  const add = (raw: string) => {
    const additions = raw
      ...
      .filter(Boolean)
      .filter((word, index, list) => list.indexOf(word) === index && !keywords.includes(word));
    ...
    onActionChange({
      ...keywordActions,
      ...Object.fromEntries(additions.map((word) => [word, targetAction])),
    });
    setDraft("");
  };

  const remove = (kw: string) => {
    const nextActions = { ...keywordActions };
    delete nextActions[kw];
    onActionChange(nextActions);
    onChange(keywords.filter((k) => k !== kw));
  };

  const moveKeyword = (kw: string, action: RuleAction) => {
    onActionChange({ ...keywordActions, [kw]: action });
  };

  const buckets = (["block", "warn", "flag"] as RuleAction[]).map((action) => ({
    action,
    words: keywords.filter((kw) => (keywordActions[kw] ?? defaultAction) === action),
  }));
      ...
      {!readOnly && (
        <div className="rounded-2xl border border-border bg-muted/30 p-2">
          <div className="flex flex-col gap-2 sm:flex-row">
            <div className="flex min-h-11 flex-1 items-center gap-2 rounded-xl border border-border bg-background px-3 focus-within:ring-2 focus-within:ring-ring/30">
            ...
            <div className="grid grid-cols-3 gap-1 rounded-xl border border-border bg-background p-1">
              {(["block", "warn", "flag"] as RuleAction[]).map((action) => {
                const cfg = ACTION_CONFIG[action];
                const isActive = targetAction === action;
                return (
                  <button
                    key={action}
                    type="button"
                    ...
                    aria-label={`Add new keywords to ${cfg.label}`}
                  >
          ...
          <p className="px-1 pt-2 text-[11px] text-muted-foreground">
            New keywords land in <strong>{ACTION_CONFIG[targetAction].label}</strong>. Drag chips
            between buckets to change what Argus does.
          </p>
        </div>
      )}

      <div className="grid gap-3 lg:grid-cols-3">
        {buckets.map(({ action, words }) => {
          const cfg = ACTION_CONFIG[action];
          const isDragTarget = dragging && (keywordActions[dragging] ?? defaultAction) !== action;
          return (
            <div
              key={action}
              onDragOver={(e) => {
              ...
              onDrop={(e) => {
                e.preventDefault();
                const kw = e.dataTransfer.getData("text/plain") || dragging;
                if (kw && !readOnly) moveKeyword(kw, action);
                setDragging(null);
              }}
              className={cn(
                "min-h-[8.5rem] rounded-2xl border p-3 transition-all duration-200",
                cfg.pastel,
                isDragTarget && "scale-[1.01] border-dashed shadow-md",
              )}
            >
              <div className="mb-3 flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <span className="grid h-8 w-8 place-items-center rounded-xl bg-white/75 shadow-sm">
                    <cfg.Icon className="h-4 w-4" />
                  </span>
                  <div>
                    <p className="text-sm font-bold">{cfg.label}</p>
                    <p className="text-[10px] leading-tight opacity-75">{cfg.desc}</p>
                  </div>
                </div>
              ...
              <div className="flex min-h-14 flex-wrap content-start gap-1.5">
                {words.length === 0 ? (
                  <button
                    ...
                    className={cn(
                      "w-full rounded-xl border border-dashed border-current/25 bg-white/40 px-3 py-3 text-left text-xs opacity-70 transition hover:bg-white/70 hover:opacity-100",
                  ...
                  words.map((kw) => (
                    <span
                      ...
                      className={cn(
                        "group inline-flex max-w-full items-center gap-1 rounded-full border border-white/80 bg-white px-2.5 py-1 text-xs font-semibold shadow-sm transition-all duration-200",
                        !readOnly &&
                          "cursor-grab hover:-translate-y-0.5 hover:shadow-md active:cursor-grabbing",
                        dragging === kw && "scale-95 opacity-50",
                      )}
                    >
                      ...
                      {!readOnly && (
                        <button
                          type="button"
                          onClick={() => remove(kw)}
                          className="rounded-full p-0.5 text-slate-400 opacity-70 transition hover:bg-slate-100 hover:text-slate-700 group-hover:opacity-100"
...
function emptyForm() {
  return {
    ...
    keywords: [] as string[],
    keyword_actions: {} as Record<string, RuleAction>,
    body: "",
    severity: "medium" as RuleSeverity,
    scope: "input" as RuleScope,
    match_logic: "any" as MatchLogic,
    domain: "",
    active: true,
    notes: "",
  };
...
function fromTemplate(t: Template): FormState {
  return {
    ...
    action: t.action,
    keywords: t.keywords,
    keyword_actions: Object.fromEntries(t.keywords.map((kw) => [kw, t.action])) as Record<
      string,
      RuleAction
    >,
    body: t.body,
...
const INSTALL_STEPS = [
  {
    ...
    body: "Find the downloaded .zip and extract it to a folder you'll keep (e.g. Documents/argus-extension).",
  },
  {
    icon: "🌐",
    title: "Open Chrome extensions",
    body: "In Chrome, go to chrome://extensions — you can also reach it via ⋮ → Extensions → Manage extensions.",
  },
  {
    icon: "🔧",
    title: "Enable Developer mode",
    body: 'Toggle "Developer mode" on (top-right of the extensions page). This lets you load local extensions.',
  },
  {
    icon: "📂",
    title: "Load the extension",
    body: 'Click "Load unpacked", then choose the folder that directly contains manifest.json (if you see README.txt next to manifest.json in Finder/Explorer, pick that folder — not the parent Downloads folder).',
  },
  {
    icon: "✅",
    title: "You're live",
    body: "The Argus shield icon appears in your toolbar. Open ChatGPT, Claude, or Gemini — your rules are active immediately.",
  },
  {
    icon: "🔄",
    title: "Rules stay in sync",
    body: "Your rules sync automatically from the dashboard every 15 minutes. No reinstall needed when you change rules.",
  },
];
      ...
      <DialogContent className="max-h-[85dvh] w-[min(560px,calc(100vw-2rem))] overflow-y-auto rounded-2xl p-0">
        <DialogHeader className="px-6 pt-6 pb-2">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {ruleTitle ? (
              <>
                Your extension for <strong>"{ruleTitle}"</strong> has downloaded. Follow these steps
                to install it.
              </>
            ) : (
        ...
        <div className="px-6 pb-6 space-y-3">
          {INSTALL_STEPS.map((step, i) => (
            <div
              key={i}
              className="flex gap-4 rounded-2xl border border-border bg-muted/30 px-4 py-3.5"
            >
          ...
          <div className="mt-2 rounded-xl bg-amber-50 border border-amber-200 px-4 py-3 text-xs text-amber-800">
            <strong>Note:</strong> Do not delete the unzipped folder — Chrome loads the extension
            from it each time.
          </div>
          ...
          <Button
            type="button"
            onClick={onClose}
            className="w-full mt-2 rounded-full bg-foreground text-background"
          >
            Got it
          </Button>
        </div>
...
async function downloadExtension(
  ruleId?: string,
): Promise<{ filename: string; ruleTitle?: string }> {
          ...
          {loading ? (
            ...
            <>
              ...
              <p className="text-[10px] text-muted-foreground leading-relaxed">
                This key is already baked into your downloaded extension. Only use it if you're
                connecting manually or building your own integration. Rotating it will require a
                fresh extension download.
              </p>
            </>
          ) : (
            <div className="space-y-2">
              <p className="text-xs text-muted-foreground">
                No key yet — one is created automatically when you download the extension.
              </p>
              <Button
                type="button"
                size="sm"
...
function DashboardPage() {
  ...
  const [instructions, setInstructions] = useState<{ open: boolean; ruleTitle?: string }>({
    open: false,
  });
  ...
  const selectRule = (r: RuleView) => {
    ...
    setForm({
      ...
      action: r.action,
      keywords: r.keywords,
      keyword_actions: r.keyword_actions ?? {},
      body: r.body,
      severity: r.severity,
      scope: r.scope,
    ...
    try {
      const result = await saveRuleFn({
        data: {
          ...
          keyword_actions: Object.fromEntries(
            form.keywords.map((kw) => [kw, form.keyword_actions[kw] ?? form.action]),
          ),
          body: form.body.trim(),
  ...
  return (
    <div className="mx-auto flex min-h-[calc(100dvh-8rem)] w-[min(1200px,calc(100%-2rem))] flex-col pb-6 pt-6 md:pt-8">
      <div className="mb-5 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
        <strong className="font-semibold">Early beta:</strong> do not use Argus with real data yet.
        Dashboard data is stored without encryption or production-grade protection.
      </div>
            ...
            <div className="animate-fade-up rounded-2xl border border-border bg-card p-6 shadow-sm sm:p-8">
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h1 className="text-xl font-bold tracking-tight">
                    {isNew ? "New rule" : (selected?.title ?? "Rule")}
                  </h1>
              ...
              <div className="mt-7 space-y-6">
                ...
                <div className="grid gap-6 sm:grid-cols-2">
                  <div className="flex flex-col gap-2">
                    ...
                    <p className="text-[11px] leading-relaxed text-muted-foreground">
                      Which side of the conversation Argus watches.
                    </p>
                  </div>
                  <div className="flex flex-col gap-2">
                    ...
                    <SegControl<MatchLogic>
                      ...
                      value={form.match_logic}
                      onChange={(v) => patch("match_logic", v)}
                      readOnly={readOnly}
                    />
                    <p className="text-[11px] leading-relaxed text-muted-foreground">
                      <strong>Any</strong> — one keyword triggers. <strong>All</strong> — every
                      keyword must appear.
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <KeywordInput
                    keywords={form.keywords}
                    keywordActions={form.keyword_actions}
                    defaultAction={form.action}
                    onChange={(kw) => patch("keywords", kw)}
                    onActionChange={(actions) => patch("keyword_actions", actions)}
                    readOnly={readOnly}
                  />
                  <p className="text-[11px] text-muted-foreground">
                    Add keywords into an action bucket, or drag them between buckets anytime.
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    {ACTION_CONFIG[form.action].desc}. Keyword-level actions override this default.
                  </p>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  />
                  <p className="text-[11px] text-muted-foreground">
                    Team-facing only — never shown in the extension.
                  </p>
                </div>
              </div>
              ...
              {showActions && (
                <div className="mt-6 flex flex-wrap items-center gap-3 border-t border-border pt-6">
                  <Button
                    type="button"
                    variant="outline"
                    className="fun-btn rounded-full"
                  ...
                  <Button
                    ...
                    onClick={() =>
                      navigate({ to: "/report/$ruleId", params: { ruleId: selectedId! } })
                    }
                  >
