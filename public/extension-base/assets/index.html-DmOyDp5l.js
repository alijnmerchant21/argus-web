import{s as a}from"./storage-CUkw2NAB.js";(function(){const i=document.createElement("link").relList;if(i&&i.supports&&i.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))s(e);new MutationObserver(e=>{for(const t of e)if(t.type==="childList")for(const r of t.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&s(r)}).observe(document,{childList:!0,subtree:!0});function n(e){const t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?t.credentials="include":e.crossOrigin==="anonymous"?t.credentials="omit":t.credentials="same-origin",t}function s(e){if(e.ep)return;e.ep=!0;const t=n(e);fetch(e.href,t)}})();async function d(){var r,p,c,f;const[l,i,n]=await Promise.all([a.getRules(),a.getConfig(),a.isEnabled()]),s=document.getElementById("root"),e=l.filter(o=>o.active).length,t=chrome.runtime.getURL("argus-logo.png");s.innerHTML=`
  ...
  `,(r=document.getElementById("switch"))==null||r.addEventListener("click",async()=>{const o=!await a.isEnabled();await chrome.runtime.sendMessage({action:"setEnabled",value:o}),d()}),(p=document.getElementById("sync-btn"))==null||p.addEventListener("click",async()=>{const o=document.getElementById("sync-btn");o.textContent="Syncing…",o.disabled=!0,await chrome.runtime.sendMessage({action:"syncNow"}),d()}),(c=document.getElementById("disconnect"))==null||c.addEventListener("click",async()=>{await chrome.storage.local.clear(),d()}),(f=document.getElementById("save-config"))==null||f.addEventListener("click",async()=>{const o=document.getElementById("api-key-input").value.trim(),u=document.getElementById("base-url-input").value.trim()||"https://argus-web.vercel.app";o&&(await chrome.runtime.sendMessage({action:"saveConfig",config:{apiKey:o,apiBaseUrl:u}}),d())})}d().catch(console.error);
